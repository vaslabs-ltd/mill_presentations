...
\begin{frame}[fragile]{Basic Spring Boot setup}
...
\begin{frame}[fragile]{Dev tooling}
  \begin{lstlisting}[language=Scala]
package build
...
object `package` extends MavenModule,PalantirFormatModule
  ...
  \end{lstlisting}
  \begin{block}{Some common dev tools}
    \begin{itemize}
      \item Linting and formatting: \url{https://mill-build.org/mill/javalib/linting.html\#_autoformatting_with_palantir_java_format}
      \item Docker: \url{https://mill-build.org/mill/contrib/docker.html}
      \item Packaging and Publishing: \url{https://mill-build.org/mill/javalib/publishing.html}
    \end{itemize}
  \end{block}
...
\begin{frame}{Mill design principles}
  \begin{itemize}
    \item Build definitions are Scala code:
      \begin{itemize}
        \item Uses best world of \textbf{OOP} and \textbf{FP}.
        \item Easily refactorable and testable; each \textbf{Task} yields a value that can be inspected, cached, reused.
      \end{itemize}
    ...
    \item Modules are composable units:
      \begin{itemize}
        \item Inheritance and composition to extend or build new functionality.
      \end{itemize}
    \item Mill gives you the tools to debug issues when creating your own extensions (e.g. slowness - check \textbf{mill-chrome-profile.json})
  \end{itemize}
\end{frame}
...
\begin{frame}{Why Android support in Mill matters}
  \begin{itemize}
    ...
    \item Android builds are inherently complex.
    \item Extending Android to support Mill brought functionality for:
    \begin{itemize}
      \item Dependency resolution with Gradle modules and resolution parameters (e.g. taking into account platform, architecture, build variant).
      \item Improved Kotlin tooling with KSP support shown previously (v1 and v2).
    \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{High-level Android pipeline in Mill (conceptual)}
  ...
  \begin{tikzpicture}[
    ...
    \node (sign)    [below=of pack] {Sign APK};
